...
export const getIsPlayingPendingChange = createSelector(
    getPlaybackState,
    playbackState => playbackState.isPlayingPendingChange
);
export const getPendingSeeks = createSelector(getPlaybackState, playbackState => playbackState.pendingSeeks);

export const getLatestPendingSeek = createSelector(getPendingSeeks, pendingSeeks =>
    pendingSeeks.length > 0 ? pendingSeeks[0] : null
);

export const getPlayedSeconds = createSelector(getPlaybackDuration, getPlayedFraction, (duration, fraction) =>
    duration !== null && fraction !== null ? duration * fraction : 0
);
